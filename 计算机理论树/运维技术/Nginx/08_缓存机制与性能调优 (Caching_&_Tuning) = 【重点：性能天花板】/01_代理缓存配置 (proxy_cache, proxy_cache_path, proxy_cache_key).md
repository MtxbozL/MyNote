...
|:--|:--|:--|:--|:--|
|`proxy_cache_valid`|`proxy_cache_valid [code ...] time;`|http、server、location|基于 HTTP 响应状态码，给缓存内容设置有效期|1. 支持`any`匹配所有状态码，可分多条规则给不同状态码设置不同时长<br><br>2. 仅对响应码 200/301/302 默认缓存，其他状态码必须显式指定|
|`proxy_cache_min_uses`|`proxy_cache_min_uses number;`|http、server、location|设置请求被缓存的最小访问次数，默认值`1`|1. 超过此次数的请求，响应才会被写入缓存<br><br>2. 用于过滤低频非热点请求，节省缓存存储空间|
|`proxy_cache_methods`|`proxy_cache_methods GET\|HEAD\|POST ...;`|http、server、location|指定哪些 HTTP 请求方法可以被缓存|1. 默认仅缓存`GET`、`HEAD`方法<br><br>2. 开启 POST 缓存必须自定义缓存键，加入`$request_body`相关变量，避免缓存错乱|
