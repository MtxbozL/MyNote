...
```bash
# 执行 Inventory 校验辅助命令
ansible-inventory --list /etc/ansible/inventory/inventory.ini

# 返回结果
(ansible_venv) ubuntu@ubuntu:/opt/ansible/ansible_venv/bin$ ansible-inventory --list /etc/ansible/inventory/inventory.ini        
{                                                                                       
    "_meta": {                                                                          
        "hostvars": {                                                                   
            "192.168.74.100": {                                                         
                ...
                "ansible_become_pass": "weiweimi4419",                                  
                "ansible_become_user": "root",                                          
                "ansible_connection": "ssh",                                            
                "ansible_host": "192.168.74.100",                                       
                "ansible_port": 22,                                                     
...
1. **主机变量查询**：`ansible-inventory --host <hostname>`，输出指定主机的所有变量（含主机级、组级、全局继承变量），验证变量优先级与继承关系：
    
    ```bash
    ansible-inventory -i /path/to/hosts --host 192.168.1.101
    ```
    
2. **目标主机列表查询**：`ansible <目标> --list-hosts`，列出目标匹配的所有主机，验证操作范围是否正确：
