...
|指令|官方语法|合法作用域|核心功能|||
|---|---|---|---|---|---|
|`allow`|`allow address|CIDR|all;`|`http`、`server`、`location`、`limit_except`|允许指定 IP/IP 段的访问请求|
|`deny`|`deny address|CIDR|all;`|`http`、`server`、`location`、`limit_except`|拒绝指定 IP/IP 段的访问请求，默认返回 403 Forbidden 状态码|
